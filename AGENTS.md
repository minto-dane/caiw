...
Malformed-input contract: every archive/loader failure exits(1) with a `die()` message — never crashes, never OOB, never silent corruption (CRC32 verified on every tensor; `c`/`d` outputs are all-or-nothing via tmp+rename with `atexit` cleanup).
