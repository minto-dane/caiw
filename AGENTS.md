...
- `c` rejects input basenames that `d` cannot write (`\`, `.`, `..`, >3000B) — an unwritable archive is never produced. Self-destruction guards: `c` refuses input/`--ref` paths equal to the output path; `d` refuses a member whose output path is the archive itself; `c` also refuses basename pairs that collide under `<name>.caiwtmp` (e.g. `a` + `a.caiwtmp`), so every emitted archive stays decodable. `-j`/​`--ref` memmoves keep argv's trailing NULL.
